...
async function main() {
  ...
  // Install to the lemmafit package directory (shared across all projects)
  const lemmafitDir = path.join(__dirname, '..');
  const installDir = path.join(lemmafitDir, '.dafny');
  const dafnyDir = path.join(installDir, 'dafny');
  const dafnyBin = path.join(dafnyDir, 'dafny');

  // Check if already installed
  if (fs.existsSync(dafnyBin)) {
    console.log('Dafny already installed.');
    return;
  }
