...
class Daemon {
  ...
  async compileModule(mod) {
    ...
    const targetCjs = path.join(this.srcDafnyDir, `${mod.outputName}.cjs`);
    ...
    const wrapperName = this.isMultiModule()
      ? `${mod.outputName}.ts`
      : 'app.ts';
    const wrapperPath = path.join(this.srcDafnyDir, wrapperName);

    const targetFlag = mod.target === 'node' ? '--node' : '--client';
    const dafny2jsArgs = [
      '--file', entryPath,
