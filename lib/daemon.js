...
class Daemon {
  constructor(projectDir, options = {}) {
    ...
    // Bundled paths (shipped with lemmafit package)
    this.dafny2jsBin = path.join(__dirname, '..', '.dafny2js', 'dafny2js');
  ...
  findDafny() {
    // Check lemmafit package directory first (shared installation)
    const lemmafitDafny = path.join(__dirname, '..', '.dafny', 'dafny', 'dafny');
    if (fs.existsSync(lemmafitDafny)) {
      return lemmafitDafny;
    }
    // Check for local Dafny in project (legacy)
    const localDafny = path.join(this.projectDir, 'node_modules', '.dafny', 'dafny', 'dafny');
    if (fs.existsSync(localDafny)) {
      return localDafny;
    }
